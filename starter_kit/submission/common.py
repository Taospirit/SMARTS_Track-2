...
class EasyCallbacks(DefaultCallbacks):
    ...
    def on_episode_step(
        ...
        **kwargs,
        ...
        epo_step_heading_error = episode.user_data["step_heading_error"]
        for _id, obs in episode._agent_to_last_raw_obs.items():
            ...
            # if obs.get("heading_errors", None) is not None:
            #     ego_step_heading_error[_id].append(obs["heading_errors"][-1][:3])
        # print(f"episode {episode.episode_id} at step {episode}: [heading-errors]: ")
